...
if [ $PUB_KEY != "**NULL**" ]; then
  echo $PUB_KEY | cat >> /home/alpha/.ssh/authorized_keys
  chown alpha:alpha /home/alpha/.ssh/authorized_keys

  echo "== Your custom public key has been added to the server"
else
...
RAND_PASS=$(base64 /dev/urandom | tr -d '/+' | dd bs=32 count=1 2>/dev/null)

usermod -p $RAND_PASS alpha
usermod -U alpha
...
echo ">> Your set password is $RAND_PASS"
